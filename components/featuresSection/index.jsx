...
const FeaturesSection = () => {
    ...
    (line, index) => (
      <React.Fragment key={index}>
        ...
        <>
          {index === 0 ? (
            <>
              <div className={style.FirstGradientLine}>
                <GradientLine width={"15.4375rem"} height=".4375rem" />
              </div>
            </>
          ) : (
            <>
              <div className={style.SecondGradientLine}>
                <GradientLine width={"18.375rem"} height=".4375rem" />
              </div>
            </>
          )}
        </>
  ...
  const subTitleWithLineBreaks = FeaturesText.subTitle
    .split("\n")
    .map((line, index) => (
      <React.Fragment key={index}>
        {line}
          ...
          <Row className={cn(style.DesktopView, style.makeItRow)}>
            <Col span={13} className={cn(style.divLeftColumn)}>
              <Image
                src={Features}
                alt="Features"
                className={cn(style.divImageContainer)}
              />
            </Col>
